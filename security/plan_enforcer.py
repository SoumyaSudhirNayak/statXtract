...
async def _load_dynamic_plan_limits(conn, plan: str) -> dict:
    ...
    Supports an admin-configurable 'testing mode' that applies reduced limits
    for validation purposes.
    """
    base = PLAN_LIMITS.get(plan, PLAN_LIMITS["free"]).copy()

    # Check if governance testing mode is active
    testing_mode = await _get_setting(conn, "governance.testing_mode", "false")
    is_testing = str(testing_mode).strip().lower() in ("true", "1", "yes", "on")

    if is_testing and plan != "admin":
        # In testing mode, use the special reduced testing limits
        test_queries = await _get_setting(conn, f"governance.test.{plan}.max_queries_per_month", None)
        test_rows    = await _get_setting(conn, f"governance.test.{plan}.max_rows_per_month", None)
        test_dl_max  = await _get_setting(conn, f"governance.test.{plan}.max_downloads_per_month", None)
        test_dl      = await _get_setting(conn, f"governance.test.{plan}.downloads_allowed", None)

        if test_queries is not None:
            base["max_queries_per_month"] = int(test_queries)
            base["max_queries_per_day"] = int(test_queries)
        else:
            base["max_queries_per_month"] = {"free": 5, "pro": 20, "enterprise": 50}.get(plan, 5)
            base["max_queries_per_day"] = base["max_queries_per_month"]

        if test_rows is not None:
            base["max_rows_per_month"] = int(test_rows)
            base["max_rows_per_day"] = int(test_rows)
        else:
            base["max_rows_per_month"] = {"free": 500, "pro": 5000, "enterprise": 25000}.get(plan, 500)
            base["max_rows_per_day"] = base["max_rows_per_month"]

        if test_dl_max is not None:
            base["max_downloads_per_month"] = int(test_dl_max)
            base["max_downloads_per_day"] = int(test_dl_max)
        else:
            base["max_downloads_per_month"] = {"free": 2, "pro": 10, "enterprise": 20}.get(plan, 2)
            base["max_downloads_per_day"] = base["max_downloads_per_month"]

        if test_dl is not None:
            base["downloads_allowed"] = str(test_dl).strip().lower() in ("true", "1", "yes")
...
async def get_and_enforce_plan_limits(conn, user_email: str, user_role: str):
    ...
    # Override with database-specific user overrides if defined
    if row["max_queries_override"] is not None:
        limits["max_queries_per_month"] = int(row["max_queries_override"])
        limits["max_queries_per_day"] = int(row["max_queries_override"])
    if row["max_rows_override"] is not None:
...
def is_export_format_allowed(limits: dict, export_format: str) -> bool:
    ...
    return export_format.lower() in allowed
